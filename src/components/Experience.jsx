...
const ExperienceCard = ({ exp }) => (
  <div 
    data-aos="fade-up"
    className="bg-black/25 backdrop-blur-md border border-white/10 rounded-3xl p-8 hover:bg-black/35 hover:shadow-[0_20px_50px_rgba(142,111,206,0.15)] hover:border-pink-500/20 transition-all duration-500 max-w-4xl mx-auto"
  >
    <div className="flex flex-col md:flex-row gap-8 justify-between">
      ...
      <div className="flex-1 md:border-r md:border-white/10 md:pr-8">
        ...
        <p className="text-pink-400 text-lg font-black tracking-wide mb-6 uppercase">
          {exp.organization}
        </p>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {exp.tech.map((t) => (
              <span 
                key={t}
                className="px-3 py-1.5 text-xs font-mono font-bold text-white bg-white/5 rounded-full border border-white/10 hover:bg-pink-500/20 hover:border-pink-500/30 transition-all duration-300 cursor-default"
              >
                {t}
              </span>
            ))}
        ...
        <ul className="space-y-4">
          {exp.responsibilities.map((resp, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className="text-pink-400 mt-1 select-none">⚡</span>
...
const Experience = () => {
  return (
    <section id="experience" className="bg-[#FF8EAF] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="flex flex-col gap-12">
          ...
          {/* Removed hardcoded Incubator block */}
        </div>
