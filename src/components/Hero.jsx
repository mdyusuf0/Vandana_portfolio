...
import heroVideo from '../assets/vandana/hero/vandana-hero.mp4';
import heroPoster from '../assets/vandana/hero/vandana-hero-poster.png';
...
const Hero = ({ onVideoLoaded }) => {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black">
      ...
      {/* Freelance Availability Button */}
      <div className="absolute top-28 md:top-32 right-6 md:right-12 z-40" data-aos="fade-down" data-aos-delay="500">
        <button
          onClick={() => {
            const msg = "Hi Vandana, I'm reaching out regarding a freelance opportunity.";
            navigator.clipboard.writeText(msg).catch(() => {});
            window.open(socialLinks.linkedin, '_blank');
          }}
          className="flex items-center gap-3 px-4 py-2.5 bg-black/40 border border-white/20 backdrop-blur-md rounded-full hover:bg-black/60 hover:border-green-500/50 hover:shadow-[0_0_15px_rgba(34,197,94,0.3)] transition-all duration-300 group cursor-pointer"
        >
          ...
          <span className="text-white text-xs md:text-sm font-bold tracking-wide group-hover:text-green-400 transition-colors">
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left w-full md:w-1/2 lg:w-[45%] max-w-xl">

          {/* Main Heading */}
          <h1 
            data-aos="fade-up"
            className="text-white text-2xl md:text-4xl lg:text-5xl font-black mb-4 tracking-tight leading-[1.15]"
          >
        ...
        <div 
          ...
          onClick={toggleVideo}
        >
          <div className="w-12 h-12 md:w-20 md:h-20 rounded-full border border-white/30 bg-black/20 backdrop-blur-md flex justify-center items-center group-hover:scale-110 group-hover:bg-[#FF8EAF] transition-all duration-500 shadow-[0_0_30px_rgba(255,255,255,0.1)] group-hover:shadow-[0_0_40px_rgba(255,142,175,0.6)]">
            {!isPlaying || isMuted ? (
              // Play Icon
