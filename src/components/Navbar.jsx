...
const HIRE_ME_MESSAGE = "Hello Vandana, I came across your portfolio and would like to connect with you regarding a professional opportunity.";
...
const Navbar = () => {
  ...
  return (
    <>
      {/* Clipboard Toast */}
      <div
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-[99999] px-6 py-3 rounded-full bg-[#FF8EAF] text-white text-sm font-bold shadow-[0_8px_30px_rgba(142,111,206,0.4)] transition-all duration-500 pointer-events-none whitespace-nowrap ${
          ...
          >
            ...
            <div className="flex gap-3 flex-wrap">
              <a
                ...
                onClick={() => setFallbackVisible(false)}
                className="flex-1 text-center px-5 py-3 rounded-full bg-[#FF8EAF] text-white font-bold hover:bg-pink-600 transition-all"
              >
                Open LinkedIn
              </a>
              <button
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="flex items-center">
            <a href="#" className="text-white text-2xl font-black tracking-tight whitespace-nowrap">
              {personalInfo.brandName}<span className="text-[#FF8EAF]">.</span>
            </a>
          </div>
          ...
          <div className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              ...
              >
                {link}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#FF8EAF] transition-all duration-300 group-hover:w-full"></span>
              </a>
            ))}
          </div>
        ...
        <div
          className={`md:hidden absolute top-full left-0 w-full transition-all duration-300 overflow-hidden ${
            isOpen ? 'max-h-96 py-4 opacity-100 bg-[#FF8EAF] shadow-2xl' : 'max-h-0 opacity-0 bg-transparent'
          }`}
        >
          <div className="flex flex-col px-6 space-y-4">
            ...
            <div className="pt-4 pb-2">
              <button
                ...
                className="inline-block px-6 py-3 rounded-full bg-white text-[#FF8EAF] font-black hover:bg-black hover:text-white transition-colors w-full text-center shadow-lg cursor-pointer"
              >
                Hire Me
              </button>
            </div>
